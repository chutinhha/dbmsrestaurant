...
namespace DAO
{
    public class MonAn_DAO
    {
       ...
       public static  DataTable DocMonAn_Begin()
        {
            Provider provider=new Provider();
            string str_sql = "select MaMon, TenMon,TenLoai,Gia,DonViTinh,TenNH";
            str_sql+= " from MonAn m,LoaiMonAn lm,NhaHang nh";
        ...
        public static int CapNhatMonAn(MonAn_DTO MonAn, int maMonAn)
        {
            Provider provider = new Provider();
            //chua xu ly nha hang
            string sql = string.Format("begin tran update MonAn set TenMon=N'{0}',Gia='{1}',DonViTinh='{2}' where MaMon='{3}'", MonAn.tenmon, MonAn.Gia, MonAn.Dvtinh, maMonAn);
